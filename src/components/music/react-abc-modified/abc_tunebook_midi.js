import tunebook from "abcjs/src/api/abc_tunebook"

import midi from "abcjs/src/midi/abc_midi_controls"
import midiCreate from "abcjs/src/midi/abc_midi_create"
...
var renderMidi = function(output, abc, parserParams, midiParams, renderParams) {
  ...
  if (midiParams) {
    ...
    for (key in midiParams) {
      if (midiParams.hasOwnProperty(key)) {
        ...
        if (key === "listener") params.midiListener = midiParams[key]
        else if (key === "transpose") params.midiTranspose = midiParams[key]
        else params[key] = midiParams[key]
      }
    }
  }
  if (renderParams) {
  ...
  if (params.generateInline === undefined)
    // default is to generate inline controls.
  ...
  function callback(div, tune, index, abcString) {
    ...
    if (params.generateInline) {
      ...
      html += midi.generateMidiControls(
        tune,
        params,
        inlineMidi,
        index,
    ...
    if (
      params.generateInline &&
      params.inlineControls &&
      params.inlineControls.startPlaying
    ) {
